...
ssh -t -i .dev.pem ubuntu@$HOST "cd $KEY/app;sed -i \"s/run roman_reactjs/run roman_reactjs_$KEY/\" \"bin/setup.sh\""
ssh -t -i .dev.pem ubuntu@$HOST "cd $KEY/app;sed -i \"s/exec roman_reactjs/exec roman_reactjs_$KEY/\" \"bin/setup.sh\""
ssh -t -i .dev.pem ubuntu@$HOST "cd $KEY/app;sed -i \"s/roman_reactjs/roman_reactjs_$KEY/\" \"bin/docker/docker-compose-dev.yml\""
